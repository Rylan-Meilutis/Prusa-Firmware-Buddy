...
def main() -> int:
    ...
    dimensions = re.search(r"font_t\s+\w+\s*=\s*\{\s*(\d+)\s*,\s*(\d+)",
                           source)
    ...
    old_bpr = re.search(
        r"font_t\s+\w+\s*=\s*\{\s*\d+\s*,\s*\d+\s*,\s*(\d+)\s*,", source)
    bytes_per_glyph = int(
        old_bpr.group(1)) * height if old_bpr else (width * height + 1) // 2
    if len(raw) % bytes_per_glyph:
        raise RuntimeError(
            f"font payload is not glyph-aligned in {sys.argv[1]}")
    ...
    for glyph_start in range(0, len(raw), bytes_per_glyph):
        ...
        while cursor < len(glyph):
            if glyph[cursor] == 0:
                ...
                while end < len(
                        glyph) and glyph[end] == 0 and end - cursor < 128:
                    end += 1
