...
Each serial-print session now starts on a neutral waiting page. Status records from the previous job are excluded, so an idle host connection cannot incorrectly show an old heating, probing, or other operation screen.
...
All 14 6.5.7 presets passed. Maximum MINI flash use was 96.69%, MK4 used
94.59%, MK3.5 used 90.03%, and XL used 69.18%. The staged release directory
...
94.63%, MK3.5 used 90.05%, and XL used 69.16%. The staged release directory
contains exactly 14 BBFs under `bbf/6.5.7`.
...
## Build 37 release update

Build 37 makes failed RME uploads diagnosable and resumable. Binary NACKs now
...
Resume metadata is persisted beside the partial file at BEGIN, allowing an
identical BEGIN to recover after USB disconnect or firmware restart. Binary
uploads now also accept CRC-protected RME control frames at reserved offset
`0xfffffffe`, so out-of-band UI, status, and recovery commands remain
responsive without placing command bytes in the uploaded file. The FILE
...
All 14 6.5.7 presets passed. Maximum MINI flash use was 96.96%, MK4 used
94.78%, MK3.5 used 90.21%, and XL used 69.31%. Exactly 14 BBFs were staged
under `bbf/6.5.7`.

Build 37 firmware continuation:

```text
914e37b26  2026-08-11  Make RME uploads durable and controllable
```
