...
static bool handle_remote_filament_service(const std::string_view command) {
  ...
  if (action.starts_with("QUERY")) {
    ...
    if (!tool || *tool < 0 || static_cast<size_t>(*tool) >= VirtualToolIndex::count) {
      SERIAL_ECHOLNPGM("echo:RME_ERROR code=invalid_filament_tool");
      return true;
    }
    ...
    if (const auto profile = remote_value(command, "profile")) {
      ...
      if (type == FilamentType::none) {
        SERIAL_ECHOLNPGM("echo:RME_ERROR code=invalid_filament_profile");
        return true;
      }
    }
    ...
    if (const auto material = remote_value(command, "material")) {
      ...
      if (*material == "none") {
        if (!type.is_customizable()) {
          SERIAL_ECHOLNPGM("echo:RME_ERROR code=immutable_filament_material");
          return true;
        }
        ...
        if (!preset) {
          SERIAL_ECHOLNPGM("echo:RME_ERROR code=invalid_filament_material");
          return true;
        }
        if (type.is_customizable()) {
          params.base_preset = *preset;
        } else if (type != material_type) {
          SERIAL_ECHOLNPGM("echo:RME_ERROR code=filament_material_mismatch");
          return true;
        }
      }
      if (type.is_customizable()) type.set_parameters(params);
#else
      SERIAL_ECHOLNPGM("echo:RME_ERROR code=unsupported_filament_material");
      return true;
#endif
    }

    config_store().set_filament_type(VirtualToolIndex::from_raw(static_cast<uint8_t>(*tool)), type);
    const auto assigned = type.parameters();
    SERIAL_ECHO("RME_FILAMENT_ASSIGNED tool="); SERIAL_ECHO(*tool);
    SERIAL_ECHO(" material="); SERIAL_ECHO(filament_material_name(assigned).data());
    SERIAL_ECHO(" profile="); SERIAL_ECHOLN(assigned.name.data());
  ...
  while ((buddy_rme_binary_upload_active() || length < recovery_capacity) && serial_data_available()) {
    for (uint8_t i = 0; i < NUM_SERIAL; ++i) {
      ...
      if (serial_char == '\n' || serial_char == '\r') {
        ...
        // the deliberately small Marlin task stack: the 6.8.x connection
        // query burst otherwise overflows it immediately.
