...
using ProgressHook = stdext::inplace_function<bool(const ProgressHookParams &params)>;
...
std::optional<ResponseSample> measure(const MeasureParams &args, float frequency, const ProgressHook &progress_hook);
