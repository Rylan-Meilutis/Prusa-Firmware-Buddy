...
- **Loaded filament editor:** The root Filament menu is the single location for loaded tool/slot assignments. Rows show material and color names without preset metadata or hex clutter. Opening a row stages separate Material and Color choices and applies both only through Save. The color chooser contains built-in colors plus only populated user-color slots. New named user colors are created under **Settings > Filament Colors**; exact hex values are reported to serial hosts by `M865`, not displayed in the loaded-filament list. MINI uses a compact material-only editor to fit its flash partition.
