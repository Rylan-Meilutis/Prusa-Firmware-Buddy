...
- **Serial firmware staging:** On MINI, MK-series, CORE-series, XL, and iX, `M998` uploads a BBF to `/usb/FWUPD.BBF` in acknowledged 48-byte Base64 chunks with strict offsets, declared size, and SHA-256 verification. Uploading never flashes automatically; a separate `M997 /usb/FWUPD.BBF` requests the bootloader handoff. MINI retains the same guided firmware-update menu as the other printers; its complete bitmap fonts are loaded and glyph-cached from the external resource image to preserve internal flash headroom without reducing language or UI coverage. See the [M998 protocol guide](doc/gcode/M998.md).
