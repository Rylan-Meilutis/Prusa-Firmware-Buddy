...
- **Persistent filament colors:** Loaded material records include a per-tool/MMU color chosen from a built-in palette or eight user-defined persistent slots. Every interactive filament load and MMU preload asks for both material and color, while unloading clears both assignments. Colors can also be changed from Loaded Filament management, set or queried with `M865`, and imported/exported with the other RME settings. Flash-constrained MINI builds show the loaded color name but manage color metadata through `M865` and RME settings import.
