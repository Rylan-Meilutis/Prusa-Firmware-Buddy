...
- **Priority serial recovery:** Two hidden G-code receive slots remain available beyond the normal streaming queue at all times, allowing an already-in-flight line plus a recovery command without sacrificing normal command ordering. This remains available during blocking heater waits before any pause/error dialog exists. Serial `M601` immediately cancels an active heat wait and requests a pause; `M876 S<n>` selects the zero-based button from the active dialog, so MMU Retry and every other displayed recovery action remain available even when the main queue is full. Direct `M1601 C/U/A`, `M524`, `M604`, `M108`, `M112`, and `M410` handling uses the same receive opportunity.
