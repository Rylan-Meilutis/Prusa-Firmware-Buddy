...
- **Priority serial recovery:** Actionable pause/error dialogs open two G-code receive slots beyond the normal streaming queue, allowing an already-in-flight line plus a recovery command without sacrificing normal command ordering. `M876 S<n>` selects the zero-based button from the active dialog, so MMU Retry and every other displayed recovery action remain available even when the main queue is full. Direct `M1601 C/U/A`, `M524`, `M604`, `M108`, `M112`, and `M410` handling uses the same receive opportunity.
