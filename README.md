...
- **Firmware picker:** **Settings > FW update** can browse the USB drive, select a root-level `.bbf`, confirm it, and restart into the bootloader update flow. The selected filename must fit the bootloader's 8.3 short-name handoff. The bootloader still validates the BBF signature and printer compatibility before flashing.
