...
- **Long serial-command reliability:** Blocking PA calibration reports immediate batch acceptance and regular Marlin busy keepalives. A retransmission of the same numbered command while it is still executing is treated as a harmless duplicate, preventing timeout-driven line-number flush/resend storms while preserving the original command's final `ok`.
  A guided **Control > Pressure Advance Calibration** screen sits immediately above Calibrations & Tests and presents every loaded tool/MMU slot as an independent on/off selection with its own test temperature, followed by one Run action. The blocking progress screen takes over as soon as the batch begins, retains the normal temperature/status footer, keeps chamber lighting active, remains visible over idle, file-print, and serial-print screens, and permits only Abort until it presents the selected tools' aggregated results. MMU runs probe before loading and unload the final slot before returning or showing results; all results can be saved together to `/usb/pa-calibration.gcode` or dismissed. Loaded-filament machines probe at the material profile's lower nozzle-preheat temperature, create 10 mm of bed/nozzle clearance, and only then heat to the test temperature; MMU machines probe unloaded with the same clearance before full heating. Each temperature remains limited to ±15 °C around that tool's assigned material profile, M976 restores every previous hotend target on all exit paths, and unattended batches wait for reachable restored targets so the nozzle is back at the template's explicit probe temperature before MBL. Results are reported to three decimal places with 0.002-second search resolution and a 0.500 safety ceiling.
