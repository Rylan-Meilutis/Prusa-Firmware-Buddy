...
- **External UI assets:** MINI and every xBuddy machine load complete bitmap fonts as independently compressed, random-access glyphs from the signed managed resource image. This preserves all language and UI coverage while leaving substantial internal application-flash headroom; MK4 release usage is about 60% instead of sitting at the partition limit. `M998` uploads a BBF to `/usb/FWUPD.BBF` in acknowledged 48-byte Base64 chunks with strict offsets, declared size, and SHA-256 verification. Uploading never flashes automatically; a separate `M997 /usb/FWUPD.BBF` requests the bootloader handoff. See the [M998 protocol guide](doc/gcode/M998.md).
