...
- **Sensor-aware unload bypass:** An unload skips heating and filament motion only when enabled, healthy sensors explicitly report an empty path (both toolhead and FINDA on MMU systems, or the active nozzle-side sensor elsewhere). Disabled or unavailable sensors always run the unload, and an automatic runout keeps unloading so the remaining filament tail is removed.
