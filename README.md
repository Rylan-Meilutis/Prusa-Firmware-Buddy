...
- **Chamber and filtration improvements:** Expanded chamber fan controls, configurable filtration behavior, Core One / Core One Plus selection with distinct `COREONE+` reporting, Core One Plus automatic vent handling that defers to explicit `M870` commands, and prompt-free serial-print vent handling.
- **PID management:** View, edit, reset, and autotune supported hotend and heatbed PID values from the Calibrations & Tests menu. Autotune displays progress and lets the user save or discard the resulting values.
...
- **Safety, UI, and maintenance refinements:** Configurable hotend and bed-heater safety timeouts under Heater Safety, both capped at 60 minutes, improved paused-print safety behavior, theme updates and theme import, lock settings, screen wake protection, and active lighting during setup, calibration, self-test, and MMU workflows.
