...
- **Long serial-command reliability:** Blocking PA calibration reports immediate batch acceptance and regular Marlin busy keepalives. A retransmission of the same numbered command while it is still executing is treated as a harmless duplicate, preventing timeout-driven line-number flush/resend storms while preserving the original command's final `ok`. Streamed-print completion does not emit the media-only `Done printing file` marker, preventing OctoPrint from resetting its line counter while older numbered status commands remain queued.
