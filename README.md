...
- **Persistent filament colors:** Loaded material records include a per-tool/MMU color chosen from a built-in palette or eight user-defined persistent slots. Colors are set during loading or from Loaded Filament management, can be set and queried with `M865`, and custom definitions are included in RME settings export/import.
  A guided **Calibrations & Tests > Pressure Advance Calibration** screen supports one loaded tool/MMU slot or all loaded filaments sequentially. It automatically selects each slot's assigned material profile; manual temperatures are limited to ±15 °C around that profile and reset when switching materials. M976 restores every previous hotend target on all exit paths.
