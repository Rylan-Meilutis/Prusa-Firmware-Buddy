...
Built-in UI theme selection applies the matching status LED palette immediately. Keep Indigo as the only built-in profile with a non-cyan printing status LED color; use the saturated LED-specific indigo `#3f00a8` so the physical strip does not read as pale cyan. The other built-in profiles use the standard cyan printing bar.
