...
Built-in UI theme selection applies the matching status LED palette immediately. Keep Indigo as the only built-in profile with a non-cyan printing status LED color; use the darker saturated LED-specific indigo `#300080` so the physical strip matches the screen palette more closely. The other built-in profiles use the standard cyan printing bar.
