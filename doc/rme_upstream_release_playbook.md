...
Built-in UI theme selection applies the matching status LED palette immediately. Keep Indigo as the only built-in profile with a non-cyan printing status LED color and use the display-matching deep indigo `#4b2e83`. The other built-in profiles use the standard cyan printing bar.
...
Keep xBuddy-extension RGBW PWM on its dedicated approximately 977 Hz TIM2 carrier. The former approximately 23.5 kHz carrier produced irregular low-duty strip response, so adjacent color values could render inconsistently or appear fully off. Convert perceptual UI RGBW intensities to linear timer duty cycles in the extension HAL before writing TIM2 compare registers so mixed strip colors match their display palette. Do not alter TIM3 fan and chamber-light timing when tuning RGBW output.
