...
Built-in UI theme selection applies the matching status LED palette immediately. Keep the Indigo printing status bar deep indigo `#4b2e83`; keep every other built-in profile explicitly cyan `#0096ff`. Imported theme JSON exposes the in-progress status strip / light-bar color as `led.printing`.
