...
Loadcell stuck-filament detection is controlled by `M591` and the `Stuck Filament Detection` GUI toggle on loadcell printers. Preserve `M591 S/P/R/I/F` behavior so users can enable, persist, restore defaults, and tune the skip debounce without requiring a rebuild.
