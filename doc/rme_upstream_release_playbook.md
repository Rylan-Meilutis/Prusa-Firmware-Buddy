...
Keep the config-version migration that upgrades the former Indigo preset's persisted cyan printing LED default to indigo. Existing installations retain store values across firmware updates, so changing only the store-item default does not update upgraded printers.
