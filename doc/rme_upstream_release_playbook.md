...
Keep xBuddy-extension RGBW PWM on its dedicated approximately 23.4 kHz TIM2 carrier and preserve raw `0-255` compare values. Do not alter TIM3 fan and chamber-light timing when tuning RGBW output. Solid `M150 A0` calibration commands must bypass the normal status-animation cross-fade so hardware measurements are stable immediately.
