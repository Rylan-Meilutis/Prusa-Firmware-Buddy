...
Built-in UI theme selection applies the matching status LED palette immediately. Keep Indigo as the only built-in profile with a non-cyan printing status LED color; the other built-in profiles use the standard cyan printing bar.
