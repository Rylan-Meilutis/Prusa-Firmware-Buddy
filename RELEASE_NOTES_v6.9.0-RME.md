...
- Integrate all seven functional fixes from upstream v6.9.1-beta: 150 C offset
  probing, ten sensor Z probe attempts, fixed-current sensor configuration and
  recovery, 100 mm calibration clearance, faster away-from-bed parking, and
  UART recovery mutex protection. Retain the RME version and custom features.
- Extend CORE One INDX vent closing from X117 to X118 mm without changing its
  approach, retreat, speed or other machine variants. Hardware testing remains
  required for complete closure without excessive lever force and calibration.
- All 15 variants built successfully from ce5dfc82a. Companion plugin b117
...
## Release update — INDX park exit, 2026-09-21
...
- The exit does not request a Z move; external serial keep-out checks remain
  unchanged. Hardware validation remains pending.

## Release update — lighting follow-up, 2026-09-21
...
- LCD On resumes the configured print lighting profile; Off is temporary.
  Print-end cleanup runs once so the normal idle timeout can expire.
- Five parking/lighting source checks pass on each branch. Shared RME tests
  pass 432641 assertions across 65 cases. Hardware validation remains pending.
- Companion RME Compatibility 0.1.0b114 exposes print brightness controls,
  adds immediate LCD pending feedback, and rejects premature zero-time estimates.

Release validation (2026-09-20): all 15 variants passed from `f7f0a470a`.
...
- Shared regression validation: 65 RME cases / 432590 assertions passed on
  6.10.1. Hardware verification and the 6.9.0 build remain pending.
...
Validation: 15/15 release variants passed from `c19566ee9`. Host suites:
...
DOM synchronization passed. Hardware validation is still pending.
...
Final build from `7867de36f`: 15/15 presets passed in 8m36s.
...
Validation: all 15 final firmware presets passed from `a7b42b0ed`.
...
Validation: all 15 final firmware presets passed from `5ee44226c`.
...
77.47% aggregate RAM; the tightest MINI flash target is 99.95%.
