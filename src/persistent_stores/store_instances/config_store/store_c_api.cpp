...
float effective_axis_steps_x() {
    const float v = config_store().axis_steps_per_unit_x.get();
    return v == config_store_ns::steps_per_unit_unset ? get_default_steps_per_unit_x_signed() : v;
}
float effective_axis_steps_y() {
    const float v = config_store().axis_steps_per_unit_y.get();
    return v == config_store_ns::steps_per_unit_unset ? get_default_steps_per_unit_y_signed() : v;
}
} // namespace
