...
char selected_media_path[filename_defs::path_buffer_size] = {};
...
void list_dir_recursive(const char *dir_path, const char *relative_dir, bool include_lfn, uint8_t depth) {
    ...
    while (struct dirent *entry = dir.read()) {
        ...
        std::array<char, filename_defs::path_buffer_size> full_path {};
        if (!join_path(full_path.data(), full_path.size(), dir_path, entry->d_name)) {
            continue;
        }

        if (entry->d_type == DT_DIR) {
            std::array<char, filename_defs::path_buffer_size> relative_path {};
            ...
            std::array<char, filename_defs::path_buffer_size> relative_lfn {};
...
const char *arg_after_command(const char *command, uint16_t code) {
    ...
    static char arg_buffer[filename_defs::path_buffer_size] = {};
    size_t i = 0;
