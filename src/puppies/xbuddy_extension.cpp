...
static constexpr uint8_t modbusUnitNr = std::to_underlying(modbus::ServerAddress::mmu);
...
CommunicationStatus XBuddyExtension::refresh_mmu() {
    ...
    case MMUModbusRequest::RW::read: {
        ...
        auto rv = bus.read_holding(modbusUnitNr, &mmuModbusRq.u.read.value, 1, mmuModbusRq.u.read.address, mmuModbusRq.timestamp_ms, 0);
        log_debug(MMU2, "read holding(uni=%" PRIu8 " val=%" PRIu16 " adr=%" PRIu16 " ts=%" PRIu32 " rv=%" PRIu8,
            modbusUnitNr, mmuModbusRq.u.read.value, mmuModbusRq.u.read.address, mmuModbusRq.timestamp_ms, (uint8_t)rv);
    ...
    case MMUModbusRequest::RW::write: {
        ...
        auto rv = bus.write_holding(modbusUnitNr, &mmuModbusRq.u.write.value, 1, mmuModbusRq.u.write.address, dirty);
        ...
        log_debug(MMU2, "write holding(uni=%" PRIu8 " val=%" PRIu16 " adr=%" PRIu16 " ts=%" PRIu32 " rv=%" PRIu8,
            modbusUnitNr, mmuModbusRq.u.write.value, mmuModbusRq.u.write.address, mmuModbusRq.timestamp_ms, (uint8_t)rv);
        if (rv == CommunicationStatus::OK) {
    ...
    case MMUModbusRequest::RW::query: {
        ...
        auto rv = bus.read(modbusUnitNr, mmuQuery, 0);
    ...
    case MMUModbusRequest::RW::command: {
        ...
        auto rv = bus.write_holding(modbusUnitNr, &mmuModbusRq.u.command.cp, 1, xbuddy_extension::mmu_bridge::commandInProgressRegisterAddress, dirty);
        if (rv != CommunicationStatus::OK) {
        ...
        rv = bus.read_holding(modbusUnitNr, &mmuQuery.value.commandStatus, 1, xbuddy_extension::mmu_bridge::commandStatusRegisterAddress, mmuModbusRq.timestamp_ms, 0);
