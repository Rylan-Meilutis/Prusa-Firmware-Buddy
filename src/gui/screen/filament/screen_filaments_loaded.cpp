...
MI_ASSIGN_LOADED_COLOR::MI_ASSIGN_LOADED_COLOR(VirtualToolIndex tool, std::optional<Color> color, std::string_view name)
    : IWindowMenuItem({}, color ? filament_color_gui::swatch_extension_width : Rect16::Width_t { 0 }, nullptr, is_enabled_t::yes, is_hidden_t::no)
    , tool_(tool)
    , color_(color) {
    #if HAS_MINI_DISPLAY()
    ...
    #else
    if (color) {
        snprintf(label_.data(), label_.size(), "%.*s", static_cast<int>(name.size()), name.data());
    } else {
        snprintf(label_.data(), label_.size(), "%s", "None");
    }
...
void WindowMenuAssignLoadedColor::setup_item(ItemVariant &variant, int index) {
    ...
    if (index == 1) {
        variant.emplace<MI_ASSIGN_LOADED_COLOR>(tool_, std::nullopt, std::string_view("None"));
        return;
    }
...
void MI_ADD_CUSTOM_COLOR::click(IWindowMenu &menu) {
    size_t slot = 0;
    while (slot < filament_color::custom_slot_count && filament_color::custom(slot)) {
        ++slot;
    }
    ...
    if (!DialogTextInput::exec(_("Color name"), name) || !DialogTextInput::exec(_("Color picker #RRGGBB"), hex, false, true)) {
        return;
    }
...
void WindowMenuCustomFilamentColors::reload_colors() {
    saved_count_ = 0;
    for (size_t slot = 0; slot < filament_color::custom_slot_count; ++slot) {
        saved_count_ += filament_color::custom(slot).has_value();
    }
    setup_items();
}
...
void WindowMenuCustomFilamentColors::setup_item(ItemVariant &variant, int index) {
    ...
    if (index == 1) {
        variant.emplace<MI_ADD_CUSTOM_COLOR>();
        return;
    }
...
MI_ASSIGN_MANUFACTURER::MI_ASSIGN_MANUFACTURER(std::optional<uint8_t> id, std::string_view name)
    ...
    , id_(id) {}
void MI_ASSIGN_MANUFACTURER::click(IWindowMenu &) {
    pending.manufacturer = id_;
    Screens::Access()->Close();
}
...
void WindowMenuAssignManufacturer::setup_item(ItemVariant &variant, int index) {
    ...
    if (index == 1) {
        variant.emplace<MI_ASSIGN_MANUFACTURER>(std::nullopt, std::string_view("None"));
        return;
    }
    size_t requested = index - 2;
    if (requested < filament_manufacturer::preset_count) {
        const auto name = filament_manufacturer::preset(requested);
        variant.emplace<MI_ASSIGN_MANUFACTURER>(static_cast<uint8_t>(requested + 1), std::string_view(name));
        return;
    }
    requested -= filament_manufacturer::preset_count;
    for (size_t slot = 0; slot < filament_manufacturer::custom_slot_count; ++slot) {
        if (const auto item = filament_manufacturer::custom(slot); item && requested-- == 0) {
            variant.emplace<MI_ASSIGN_MANUFACTURER>(item->id, item->name_view());
            return;
        }
    }
}
ScreenAssignManufacturer::ScreenAssignManufacturer()
    : ScreenMenuBase(nullptr, _("ASSIGN MANUFACTURER"), EFooter::Off) {}
...
MI_SAVED_CUSTOM_MANUFACTURER::MI_SAVED_CUSTOM_MANUFACTURER(size_t slot)
    ...
    if (const auto item = filament_manufacturer::custom(slot)) {
        label_ = item->name;
        SetLabel(string_view_utf8::MakeRAM(label_.data()));
    }
}
...
void WindowMenuCustomManufacturers::reload() {
    saved_count_ = 0;
    for (size_t i = 0; i < filament_manufacturer::custom_slot_count; ++i) {
        saved_count_ += filament_manufacturer::custom(i).has_value();
    }
    setup_items();
}
...
void WindowMenuCustomManufacturers::setup_item(ItemVariant &variant, int index) {
    ...
    if (index == 1) {
        variant.emplace<MI_ADD_CUSTOM_MANUFACTURER>();
        return;
    }
    size_t requested = index - 2;
    for (size_t slot = 0; slot < filament_manufacturer::custom_slot_count; ++slot) {
        if (filament_manufacturer::custom(slot) && requested-- == 0) {
            variant.emplace<MI_SAVED_CUSTOM_MANUFACTURER>(slot);
            return;
        }
    }
}
ScreenCustomManufacturers::ScreenCustomManufacturers()
    : ScreenMenuBase(nullptr, _("FILAMENT MANUFACTURERS"), EFooter::Off) {}
#endif
