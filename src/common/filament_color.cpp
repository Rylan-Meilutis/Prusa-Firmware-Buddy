...
std::optional<Profile> custom(const size_t slot) {
    if (slot >= custom_slot_count || !(config_store().custom_filament_color_valid.get() & (1u << slot))) {
        return std::nullopt;
    }
...
bool set_custom(const size_t slot, const std::string_view name, const Color color) {
    if (slot >= custom_slot_count || name.empty() || name.size() >= name_capacity) {
        return false;
    }
...
std::optional<Color> loaded(const uint8_t tool) {
    if (tool >= VirtualToolIndex::count || !(config_store().loaded_filament_color_valid.get() & (1u << tool))) {
        return std::nullopt;
    }
...
void set_loaded(const uint8_t tool, const std::optional<Color> color) {
    if (tool >= VirtualToolIndex::count) {
        return;
    }
...
Profile profile_for(const Color color) {
    ...
    for (size_t slot = 0; slot < custom_slot_count; ++slot) {
        if (const auto profile = custom(slot); profile && profile->color == color) {
            return *profile;
        }
    }
    Profile result { .color = color };
