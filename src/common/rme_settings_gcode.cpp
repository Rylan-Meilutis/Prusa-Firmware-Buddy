...
bool write(FILE *file) {
    ...
    ok &= fprintf(file, "M976 Q%.2f N%.1f R%u D%u\n",
              ...
              config_store().pa_calibration_debug_output.get() ? 1 : 0)
        >= 0;
    ...
    ok &= fprintf(file, "M154.0 D%u I%u A%u P%u\n",
              ...
              packed_screen_percent(screen, leds::LightState::printing))
        >= 0;
    ...
    ok &= fprintf(file, "M154.1 D%u I%u A%u P%u\n",
              ...
              pwm_to_percent(config_store().side_leds_print_brightness.get()))
        >= 0;
    ok &= fprintf(file, "M154.3 A%u E%u O%u H%u F%u\n",
              ...
              config_store().post_print_led_hold_enabled.get() ? 1 : 0)
        >= 0;
#endif
    ...
    ok &= fprintf(file, "M154.2 D%u I%u A%u P%u\n",
              ...
              packed_percent(status, leds::LightState::printing))
        >= 0;
    ...
    ok &= fprintf(file, "M154.4 U%u T%u A%u S%u\n",
              ...
              config_store().serial_print_screen_enabled.get() ? 1 : 0)
        >= 0;
    ...
    ok &= fprintf(file, "M154.5 D%u I%u A%u P%u\n",
              ...
              leds::external_light_bar::state_enabled(leds::LightState::printing) ? 1 : 0)
        >= 0;
#endif
    ...
    for (size_t slot = 0; slot < filament_color::custom_slot_count; ++slot) {
        if (const auto profile = filament_color::custom(slot)) {
            ok &= fprintf(file, "M865 V%u O\"#%06lx\" N\"%.*s\"\n", static_cast<unsigned>(slot),
                      static_cast<unsigned long>(profile->color.raw & 0xffffff), static_cast<int>(profile->name_view().size()), profile->name.data())
                >= 0;
        }
    }
    ...
    for (size_t slot = 0; slot < filament_manufacturer::custom_slot_count; ++slot) {
        if (const auto profile = filament_manufacturer::custom(slot)) {
            ok &= fprintf(file, "M865 W%u N\"%.*s\"\n", static_cast<unsigned>(slot),
                      static_cast<int>(profile->name_view().size()), profile->name.data())
                >= 0;
        }
    }
