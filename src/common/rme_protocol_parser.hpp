...
inline std::optional<long> signed_number(const std::string_view command, const std::string_view key, const int base = 10) {
    ...
    if (!text || text->empty() || text->size() >= 16) {
        return std::nullopt;
    }
...
inline bool is_service_frame(const std::string_view command) {
    ...
    if (command[start] == 'N') {
        ...
        while (start < command.size() && command[start] >= '0' && command[start] <= '9') {
            ++start;
        }
        if (start == digits) {
            return false;
...
inline std::optional<std::array<char, Capacity>> usb_path(const std::string_view encoded) {
    ...
    size_t read = 0;
    while (read < encoded.size() && encoded[read] == '/') {
        ++read;
    }
    ...
    if (normalized.find("..") != std::string_view::npos || normalized.find("//") != std::string_view::npos) {
        return std::nullopt;
    }
    return path;
}
